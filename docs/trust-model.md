...
The model-facing tool is `cmd`. Lucy authorizes the model to choose command text; this is an authority grant, not a claim that the choice is safe. Repository content, instructions, skills, dependency documentation, command output, compiler and test output, HTTP responses, generated files, and output from other agents are untrusted inputs that can influence that choice.
...
- session records written by appending JSONL records rather than rewriting earlier history;
- bounded provider SSE streams and provider response fields where source constants impose limits; there is no general total-session-size bound;
...
Lucy does not provide filesystem or network sandboxing, a container, a virtual machine, a separate OS user, repository-root confinement, semantic command analysis, guaranteed prompt-injection resistance, guaranteed protection of unrelated credentials, or guaranteed prevention of command-policy bypass. It does not coordinate or lock multiple processes writing the same persisted session. Its append write pattern is not encryption, tamper resistance, or a concurrent-writer guarantee.
...
No configured policy means commands are allowed by default. When a valid user-owned policy hook is loaded, it is evaluated before process spawn and can deny a command. Project instructions, skills, and model output cannot directly reconfigure Lucy's loaded policy path. That is not protection from an allowed user-privileged command: it may modify an owner-writable policy file or dependency for later commands in the same process, or modify configuration for a later run. Protect those files with external permissions or isolation when that matters. Policy path-resolution errors are currently treated as no loaded policy; `lucy doctor` does not inspect the policy. Once a valid hook is loaded, hook execution errors deny the command.
...
See the [command-policy contract](command-policy.md) for the hook protocol and current loading limitation. See [diagnostics](diagnostics.md) for what `lucy doctor` checks and what network, process, and credential effects it may have.
...
OAuth access and refresh tokens are stored in a private Lucy credential file under the resolved XDG data/config boundary. Lucy does not inject them into model-tool environment variables or write them to ordinary configuration or persisted sessions. This is not isolation from the invoking user: an unsandboxed command may read the credential file when that user can read it. OAuth exchange failures avoid response bodies, and refresh failures use generic or active-token-redacted messages. `lucy codex logout` removes the current credential file; it does not delete sessions or other local data.
...
Configuration is stored at `$XDG_CONFIG_HOME/lucy/config.toml`, falling back to `~/.config/lucy/config.toml`, with a private directory and `0600` file. Codex credentials use a private `codex-credentials.json` path resolved from `XDG_DATA_HOME`, then `XDG_CONFIG_HOME`, with `~/.config` as the fallback; diagnostics do not print the file contents. Sessions are private append-only JSONL files under `~/.lucy/sessions/`.

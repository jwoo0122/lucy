...
- The configured OpenRouter API-key environment variable may remain available to non-command discovery helpers, but is removed from `cmd` shells before spawn; serialized output and persisted records still redact it. Codex subscription tokens are never placed in child environments.

...
- The configured OpenRouter API-key environment variable remains available to trusted context-discovery helpers but is removed from `cmd` child environments before spawn; serialized output and persisted records still redact it. Codex subscription tokens are never placed in child environments.

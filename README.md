...
- **Context and skills:** New sessions receive only minimal built-in guidance, the working directory, operating system, README files, and global/project `AGENTS.md` files. Skills remain discoverable for explicit slash-prefixed invocation through saved snapshots, but their catalog is not injected into the boot prompt.

...
- **Tool activity UI:** Renders `cmd` as a compact one-line status card and keeps active background subagents in a full-width read-only task list immediately above the message input; completed workers are removed after their result is delivered. The main-agent ready/working indicator appears in the bottom status line, and the prompt border uses a left-to-right teal-to-green gradient.
